    ...
    public class ReportssController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetProductReport(int reportType, string? productName)
        {
            try
            {
                ...
                //var path = @"E:\master Backup\rdlc\ATL_INV_WebApp\Atl_Inv_Server\Reports\testproductrpt.rdlc";
               // var path = @"ATL_INV_WebApp\Atl_Inv_Server\Reports\testproductrpt.rdlc";
                Dictionary<string, string> parameters = new Dictionary<string, string>();
                parameters.Add("param", productName);
        ...
        [HttpGet]
        [Route("GetQuotaionReport")]
        public async Task<IActionResult> GetQuotaionReport(int reportType, string? quotationNo)
        {
            try
            {
                var dt = await _reportRepository.GetQuotationReport(quotationNo);
                if (dt == null || !dt.Any())
                {
                    return BadRequest("No data available for the report.");
                }
                ...
                var path = $"{this._webHostEnvironment.WebRootPath}\\Reports\\Quotation.rdlc";
                //var path = @"E:\master Backup\rdlc\ATL_INV_WebApp\Atl_Inv_Server\Reports\testproductrpt.rdlc";
                // var path = @"ATL_INV_WebApp\Atl_Inv_Server\Reports\testproductrpt.rdlc";
                Dictionary<string, string> parameters = new Dictionary<string, string>();
                parameters.Add("param", quotationNo);
                LocalReport localReport = new LocalReport(path);
                localReport.AddDataSource("QuotationDs", dt);
